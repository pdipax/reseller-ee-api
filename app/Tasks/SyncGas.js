...
class SyncGas extends Task {
  static get schedule() {
    //return '55 0 * * *'
    return '15 7,8,9,10,11,12,14,15,16,17,18,19,20 * * *'
  }
  ...
  async handle() {
    ...
    //const lastFile = await Flow.query().where('commodity', 'g').orderBy('created_at', 'desc').first()
    //const lastSync = lastFile ? moment(lastFile.created_at).subtract(30, "days") : moment('2005-01-01T21:13:08.134Z')
            ...
            if (file.path.split('.').pop().toLowerCase() == 'xml') {
              ...
              if (obj && DatiPdR && DatiPdR.length > 1) {
                ...
                for(var i = 0; i< DatiPdR.length; i++) {
                  ...
                  await FlowCombination.create(
                    {
                      flow_id: flow.id,
                      pdp: findVal(el, 'cod_pdr'),
                      cod_prat_utente: findVal(el, 'cod_prat_utente'),
                      cod_prat_distr: findVal(el, 'cod_prat_distr'),
                      matr_mis: findVal(el, 'matr_mis'),
                      details: tmpJson,
                      ref_date: ref_date || flow.created_at
                ...
                await FlowCombination.create(                 
                  {
                    flow_id: flow.id,
                    pdp: findVal(json, 'cod_pdr'),
                    cod_prat_utente: findVal(json, 'cod_prat_utente'),
                    cod_prat_distr: findVal(json, 'cod_prat_distr'),
                    matr_mis: findVal(json, 'matr_mis'),
                    details: json,
                    ref_date: ref_date || flow.created_at
                  }
                )
